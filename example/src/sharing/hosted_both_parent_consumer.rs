//! Regression test (sync runtime): a downstream `Hosted` dep that
//! takes `&Owner` for a `#[test_dep(scope = Hosted, worker = both(Trait))]`
//! owner must resolve cleanly on the parent side.
//!
//! Mirror of
//! [`example-tokio/src/sharing/hosted_both_parent_consumer.rs`](../../../example-tokio/src/sharing/hosted_both_parent_consumer.rs)
//! for the sync runtime. Before the fix the macro stored an
//! `Arc<HostedBothShared>` under the owner dep key, while the
//! generated owner getter unconditionally downcasted to
//! `Arc<OwnerType>`. Any parent-side consumer that read `&Owner` back
//! from the dependency map (for example to compute a second Hosted
//! dep's descriptor) panicked with
//! `Dependency type mismatch: Any { .. }` at
//! `collect_parent_shared_dependencies_sync`.
...
mod tests {
    ...
    #[test]
    fn downstream_hosted_dep_reads_both_owner(o: &BothOwner, d: &Derived) {
    ...
    #[test]
    fn rpc_view_still_works_alongside_downstream_hosted_consumer(ctrl: &BothOwnerControlStub) {
        let id = ctrl.next_id();
        assert!(
            id >= 1,
